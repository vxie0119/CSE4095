...
DELETE_OBJECT_URL = f"{API_URL}/{BUCKET}/{OBJECT_NAME}"
...
def test_add_object():
    ...
    post_data = create_post_data_for_post_object(BUCKET, OBJECT_NAME, file_content)
    response = post(POST_OBJECT_URL, post_data)
    assert response['statusCode'] == 200

def test_delete_object():
    data = create_data_for_del_object(BUCKET, OBJECT_NAME)
